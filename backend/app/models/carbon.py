...
from app.core.security import DeviceId
...
class CarbonResult(BaseModel):
    ...
    # -- Lifestyle context (echoed from CarbonInput) -------------------------
    # Carried on the result so insight generation can personalise to the
    # user's actual diet, shopping, and flight habits — not just the kg totals.
    diet_type: Literal["meat_heavy", "meat_medium", "vegetarian", "vegan"] = Field(
        default="meat_medium",
        description="User's dietary pattern, echoed from the original input",
    )
    consumption_level: Literal["high", "medium", "low"] = Field(
        default="medium",
        description="User's goods consumption pattern, echoed from the original input",
    )
    flights_short_haul: int = Field(
        default=0, ge=0, le=50, description="Annual short-haul flights, echoed from input"
    )
    flights_long_haul: int = Field(
        default=0, ge=0, le=20, description="Annual long-haul flights, echoed from input"
    )
