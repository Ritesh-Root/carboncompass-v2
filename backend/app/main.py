"""
CarbonCompass — FastAPI application entry point.
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    logging.getLogger(__name__).info(
        "CarbonCompass starting up (env=%s, gemini=%s, firestore=%s)",
        settings.ENVIRONMENT,
        settings.USE_GEMINI,
        settings.USE_FIRESTORE,
    )
    yield
    logging.getLogger(__name__).info("CarbonCompass shutting down")
...
# ---------------------------------------------------------------------------
app = FastAPI(
    title="CarbonCompass API",
    description=(
        "CarbonCompass — understand, track, and reduce your personal carbon "
        "footprint with science-backed estimates and AI-assisted reduction actions."
    ),
    version="1.0.0",
    docs_url="/api/docs",
    redoc_url="/api/redoc",
