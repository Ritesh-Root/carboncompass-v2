...
Uses the Vertex AI SDK (google-cloud-aiplatform / vertexai) to call the
configured Gemini model (settings.GEMINI_MODEL) and generate 3 structured,
quantified reduction actions tailored to the user's actual emission profile.
"""
...
async def generate_insights_gemini(
    ...
    """Call Vertex AI Gemini to generate personalized carbon insights.
    ...
    Raises:
        GeminiUnavailableError: If Gemini errors, returns invalid JSON, times out,
            or produces fewer than 3 valid insights (caller falls back to the rule
            engine, which always returns 3).
    """
    ...
    try:
        ...
        prompt = _build_prompt(ranked_categories, total_kg)
        ...
        if not isinstance(raw_insights, list) or len(raw_insights) < 3:
            # The response contract is exactly 3 insights. Fewer than 3 (or a
            # non-list) is treated as unavailable so we fall back to the rule
            # engine rather than return a short, contract-violating response.
            raise ValueError(
                f"Gemini returned {len(raw_insights) if isinstance(raw_insights, list) else 0} "
                "insights; need at least 3"
            )

        # Parse and validate exactly 3 insights through Pydantic
